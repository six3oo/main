...
=== Enhanced select command (Shows profile)

In its initial form, the select command shows the Google results of the contact’s name. With the application targeting a more specific group of users, YouTubers, the team believes that the select command can be better used to display the profile of the YouTuber instead. The select command opens the profile of the contact selected. The profile of the contacts displays various dynamic and static information. The static (manually changed by user) information is entered by the user at time of adding contact (refer to add command). The dynamic information is updated automatically by the application using the YouTube Data API.
...
==== Profile GUI
Displayed at the top are the dynamic information about the channel, that the contact is affiliated with, as stated above. Displayed at the bottom would be the static information of the individual contact (Figure 3.3.2.1). +


image::profile_ui.png[width="600"]
_Figure 3.3.2.1 : Positions of the different categories of information_

==== Planned extensions for the feature
The ability for the user to choose which data to display for their contacts would be implemented in future updates to increase customisability. Until then, the team has chosen what we believe are categories which YouTubers track and celebrate achievements in, for each other. This is done, after careful deliberation and consideration. Subscriber count was chosen to monitor progress towards the various YouTube Play Buttons. Video view count is the indicator of the number of people they have reached and is an important factor in channel success.
